...
生成时间 2026-09-11T08:58:04.223Z。行号 = `src/lib/system-prompt.v1.ts`（与 brief 的 L 号一致）。

- v2 中文非空行 **1964**：追溯到 v1 的 **1917**，C1–C8 新增 **47**，追溯不到 **0**。
- v1 内容行未进 v2 的 **73**：有 C 号依据 73，无依据 0。
- 三张表：合并 339 行、删除 73 行、新增 47 行。
...
| L2118–2122 | tiers.ts L129–133 | 5 |
| L2124 | tiers.ts L135 | 1 |
...
| L2536–2538 | tiers.ts L243–245 | 3 |
| L2540 | tiers.ts L247 | 1 |
| L2543–2547 | tiers.ts L249–253 | 5 |
| L1989 | tiers.ts L255 | 1 |
| L1296–1309 | tiers.ts L257–269 | 14 |
...
| L2691–2696 | tiers.ts L355–360 | 6 |
| L2698–2712 | tiers.ts L362–376 | 15 |
...
| tiers.ts L134 | addendum-B1 | - **不开礼佛** (介绍时可以点名；太难,会吓跑) | L2123 |
...
| tiers.ts L246 | addendum-B5 | 2. **首轮：同一条回复里给暂定功课，并问有没有念过经** (决定 Tier) | L2539 |
| tiers.ts L248 | C1 | 4. **礼佛从 1 遍开始** | L2541 去掉「(Tier 1 才给)」 |
| tiers.ts L271 | addendum-B1 | **4. 三大支柱（介绍时三部都要提到；首轮功课可以只开两部）：** | L1311 |
...
| tiers.ts L361 | addendum-B2 | comfortable with these two — there's no fixed timeline — you can add the Repentance Mantra. | L2697 删「(1-2 weeks)」 |
...
| relationships.ts L283 | addendum-A6 | > 📿 《大悲咒》每天 3 遍 | 没念过档功课块（祈求词行照 v1 L1788/1791/1799 原文，自动追溯） |
| relationships.ts L285 | addendum-A6 | > 📿 《心经》每天 3 遍（晚上 10 点后不念） | 没念过档功课块 |
