...
function MemberProfile({ me, id }: { me: ErpMe; id: string }) {
  ...
  if (!data) return <p className="max-w-4xl mx-auto px-4 py-10 text-sm text-[#8B6F47]">无法加载该会员。</p>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-2xl font-bold text-[#583A0F]">{m.name_cn || m.name_en || '（无名）'}</h2>
            {m.disciple === true && <Badge>弟子</Badge>}
            {m.full_veg === true && <Badge>全素</Badge>}
            {m.status === 'active'
              ? <span className="text-[11px] px-2 py-0.5 rounded-full bg-[#FAEFD0] text-[#8B6F47]">在册</span>
          ...
          {canEdit && (
            <>
              <Link href={`/dashboard/members/${id}/edit`}
                className="px-4 py-1.5 text-sm text-[#583A0F] border border-[#EFE3BF] rounded-full hover:bg-[#FAEFD0] transition">编辑</Link>
              <button onClick={toggleStatus} disabled={statusBusy}
                className={`px-4 py-1.5 text-sm rounded-full border transition disabled:opacity-50 ${
                  m.status === 'active' ? 'text-red-700 border-[#FCA5A5] hover:bg-[#FEF2F2]' : 'text-[#A87929] border-[#EFE3BF] hover:bg-[#FAEFD0]'
                }`}>
      ...
      {/* teams */}
      <Card title="组别 Teams" action={canEdit ? <button onClick={() => setTeamsOpen(true)} className="text-xs text-[#A87929] hover:underline">编辑组别</button> : undefined}>
        <div className="space-y-2">
          <div className="flex flex-wrap gap-1.5">
            {data.teams.current.length === 0 && <span className="text-sm text-[#8B6F47]">暂无当前组别</span>}
            {data.teams.current.map((t) => (
              <span key={t.team_id} className={`inline-block px-2.5 py-1 rounded-full text-xs ${
                t.role === 'lead' ? 'bg-[#F5E1B0] text-[#8A5A1E] font-medium' : 'bg-white border border-[#EFE3BF] text-[#583A0F]'
              }`}>{t.name_cn}{t.role === 'lead' ? ' · 组长' : ''}</span>
            ))}
          </div>
          {data.teams.past.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              <span className="text-[11px] text-[#B89968] mr-1">曾在：</span>
              {data.teams.past.map((t) => (
                <span key={t.team_id} className="inline-block px-2 py-0.5 rounded-full text-[11px] bg-white border border-[#EFE3BF] text-[#B89968]">{t.name_cn}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <Card title="专长 Skills">
          <div className="flex flex-wrap gap-1.5">
            {data.skills.map((sk, i) => (
              <span key={i} className="inline-block px-2 py-0.5 rounded-full text-[11px] bg-[#FAEFD0] text-[#8B6F47]" title={sk.source ?? ''}>{sk.skill}</span>
            ))}
          </div>
        </Card>
      )}
      ...
      {m.notes && (
        <Card title="备注 Notes">
          <p className="text-sm text-[#583A0F] whitespace-pre-wrap leading-relaxed">{m.notes}</p>
        </Card>
      )}

      <div className="pt-2 text-xs text-[#B89968]">
...
function TeamsDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      <div className="bg-[#FFFEF6] rounded-2xl w-full max-w-md max-h-[80vh] overflow-y-auto p-5" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-base font-semibold text-[#583A0F] mb-3">编辑组别 Teams</h3>
        <div className="space-y-1.5">
          {teams.map((t) => {
            const cur = sel[t.id] ?? { on: false, role: 'member' as const };
            return (
              <div key={t.id} className="flex items-center gap-2">
                <label className="flex items-center gap-2 flex-1 text-sm text-[#583A0F]">
                ...
                {cur.on && (
                  <select value={cur.role}
                    ...
                    className="text-xs px-2 py-1 border border-[#EFE3BF] rounded bg-white text-[#583A0F]">
        ...
        <div className="mt-4 flex items-center gap-2">
          <button onClick={save} disabled={saving}
            className="px-4 py-1.5 text-sm text-white bg-[#D89938] rounded-full hover:bg-[#A87929] disabled:opacity-50">
            {saving ? '保存中…' : '保存'}
          </button>
          <button onClick={onClose} disabled={saving}
            className="px-4 py-1.5 text-sm text-[#583A0F] border border-[#EFE3BF] rounded-full hover:bg-[#FAEFD0]">取消</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="bg-[#FFFEF6] border border-[#EFE3BF] rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-base font-semibold text-[#583A0F]">{title}</h3>
        {action}
      </div>
      {children}
    </section>
    ...
    <dl className="grid gap-x-6 gap-y-2 sm:grid-cols-2">
      {items.map(([k, v]) => (
        <div key={k} className="flex gap-2">
          <dt className="text-xs text-[#B89968] w-20 shrink-0 pt-0.5">{k}</dt>
          <dd className="text-sm text-[#583A0F] break-words">{v}</dd>
        </div>
      ))}
    </dl>
  );
