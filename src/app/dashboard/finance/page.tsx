...
type Payment = {
  ...
  months_from: string; months_to: string; channel: string; note: string | null; voided_at: string | null;
...
function Ledger({ me }: { me: ErpMe }) {
  ...
  const loadLedger = useCallback(() => {
    if (!centreId) return;
    setLoading(true);
    fetch(`/api/dashboard/finance/ledger?centre_id=${encodeURIComponent(centreId)}&year=${year}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
        if (j) {
...
function MemberPanel({ member, payments, canEdit, onClose, onChanged, onMemberUpdated }: {
  member: Member; payments: Payment[]; canEdit: boolean; onClose: () => void; onChanged: () => void;
