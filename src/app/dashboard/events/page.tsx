...
import { eventTypeLabel, eventTypeOptions, eventStatusLabel, STATUS_LABELS, STATUS_STYLES, feeLabel, moneyRM } from '@/lib/events-display';
    ...
    <div className={`${PAGE_WIDE} space-y-5`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-baseline gap-2">
          <h2 className="text-xl font-bold font-serif text-ink">{t('events.overview.title')}</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Kpi label={t('events.kpi.ongoing')} value={kpis.ongoing} icon="🟢" />
        <Kpi label={t('events.kpi.pending')} value={kpis.pending} icon="🔔" accent={kpis.pending > 0} />
        <Kpi label={t('events.kpi.monthRegs')} value={kpis.monthRegs} icon="📝" />
        <Kpi label={t('events.kpi.monthEvents')} value={kpis.monthEvents} icon="📅" />
      </div>

      {/* filters */}
      <div className="flex flex-wrap items-center gap-2">
        <Sel value={status} onChange={setStatus} options={[['', t('events.filter.allStatus')], ...Object.keys(STATUS_LABELS).map((k) => [k, eventStatusLabel(k, t)] as [string, string])]} />
        <Sel value={type} onChange={setType} options={[['', t('events.filter.allType')], ...eventTypeOptions(t)]} />
        <Sel value={centre} onChange={setCentre} options={[['', t('events.filter.allCentre')], ...centres.map((c) => [c.code, c.name_cn] as [string, string])]} />
        <input
          type="search"
          value={search}
      ...
      {/* cards */}
      {loading ? (
        <p className="p-6 text-sm text-ink-muted">{t('events.loading')}</p>
      ) : filtered.length === 0 ? (
        <div className="p-10 text-center bg-surface-soft border border-border rounded-2xl">
          <p className="text-2xl mb-1">🪷</p>
          <p className="text-sm text-ink">{events.length === 0 ? t('events.empty.none') : t('events.empty.noMatch')}</p>
          {events.length === 0 && canEdit && (
            <p className="mt-1 text-xs text-ink-muted">{t('events.empty.hint')}</p>
          )}
        </div>
      ) : (
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-ink-muted">
        <span className="inline-block px-2 py-0.5 rounded-full pill-gold">{eventTypeLabel(e.event_type, t)}</span>
      ...
      <p className="mt-1 text-xs text-ink-muted">
        ...
        {e.reg_deadline ? ` · ${t('events.card.deadline', { date: e.reg_deadline })}` : ''}
      </p>
      ...
      <div className="mt-2 flex items-center gap-3 text-[11px]">
        <span className={e.regCounts.pending > 0 ? 'text-red-700 font-medium' : 'text-ink-muted'}>{t('events.short.pending')} {e.regCounts.pending}</span>
        <span className="text-ink-muted">{t('events.short.approved')} {e.regCounts.approved}</span>
        <span className="text-ink-muted">{t('events.short.rejected')} {e.regCounts.rejected}</span>
      </div>
      ...
      {e.fees.length > 0 && (
        <p className="mt-2 text-[11px] text-ink-muted truncate" title={e.fees.map((f) => `${f.label_cn || feeLabel(f.item, t)} ${moneyRM(f.amount)}`).join(' · ')}>
          {e.fees.map((f) => `${f.label_cn || feeLabel(f.item, t)} ${moneyRM(f.amount)}`).join(' · ')}
        </p>
      )}
    </Link>
  );
