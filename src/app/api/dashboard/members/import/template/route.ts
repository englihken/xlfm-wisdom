...
const EXAMPLES: string[][] = [
  ['', '（示例）王小明', 'Example Wong', 'M', '1968-01-31', '0123456789', 'ming@example.com', '', '是', 'D1234', '2015', '是', '2016', 'L', '教师', 'volunteer', '示例行：导入时自动跳过，可整行删除'],
  ['', '', '(Example) Mary Lee', 'F', '', '6581234567', '', '', '否', '', '', '否', '', 'S', '', '', '示例行：导入时自动跳过，可整行删除'],
];
...
export async function GET() {
  ...
  ref.getCell('H1').value = '说明：本表为下拉选项参考，请勿修改。共修会名称以中文为准（导入也接受英文名/代码）。';
  ref.getRow(1).font = { bold: true };
  for (const col of ['A', 'C', 'D', 'E', 'F']) ref.getColumn(col).width = 16;
