...
评审标准（verdict）：
- "good"：回复扎实——内容有依据、切题、语气得当、访客的问题得到了回应。**功课／方法类提问（念什么经、怎么念、小房子怎么做等），good 的门槛是回复至少含一项可执行内容：具体经名 + 遍数（或张数）+ 怎么做。**
- "ok"：可用但有明显改进空间
- "needs_improvement"，出现以下任一情况：
  a) 内容错误或无依据、应当承认"查不到原文"却给出了含糊/编造的说法（反编造，照旧）
  b) 漏答了访客明确提出的问题、语气或处理不当（照旧）
  c) 不该拒答却拒答：回复出现「查不到相关原文」「没有写明具体数字」「不敢乱给数字」「建议咨询义工」之类，但〔本回复的检索来源〕里列出了相关书目（如《心灵法门入门手册》《佛学问答175问》《小房子念诵指南》），或问题本身属于基础功课范围（入门功课遍数、大悲咒／心经／礼佛三大支柱、小房子基本规格与组合）
  d) 功课不可执行：教访客念经，却没有给出经名，或没有给出遍数／张数
  e) 前后矛盾：前半段给了具体遍数或张数，后半段又说"查不到原文／不敢给数字"
  f) 转介代替回答：「联系共修会／义工」是回复的主体，而不是答完之后的结尾邀请
  g) 用「回向」教做法（「回向给谁」「功德回向」「回向文」），或把回向当成必做的动作——本法门念前祈求保佑谁就是直接回向（《佛学问答》第 77 问）
  h) 教访客为附近／周围／宿舍／学校／医院／坟场等地方的灵性、孤魂野鬼或「一切众生」念经、超度、挡；往生咒被说成超度自己以外的对象（身上的灵性、要经者、亡人、亲人）——往生咒只超度因自己而死去的小灵性
  i) 把自己的推断、常识或别的法门的说法说成「台长说过／教过／开示过」（内容不像师父的教法、或与〔本回复的检索来源〕的书目无关）
注意：a) 与 c) 并不冲突——检索来源里**没有**相关书目、问题也不属于基础功课时，承认查不到是对的（good/ok）；来源里**有**书目却仍拒答，才是 c)。
...
question_key：给对话的核心问题一个简短的规范化中文主题词（格式"主题-子题"，例如 小房子-尺寸 / 失眠-功课 / 解梦-亡人 / 礼佛-遍数 / 设佛台-位置）。闲聊或无实质问题时可用 闲聊-测试。判为 g) h) i) 时 question_key 固定用 教义纠正-回向 / 教义纠正-超度附近灵性 / 教义纠正-自创开示（多条时取最严重的一条）。
