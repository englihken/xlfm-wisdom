...
export function levelContextBlock(turn: Pick<TurnLevel, 'level' | 'needsCare'>): string {
  ...
  if (turn.level === 'experienced') {
    ...
    parts.push('【小房子张数】若访客问「要念多少张」：**必须先写出《心灵法门入门手册》的一般说法**（一般初学者先念 4-10 张小房子给自己的要经者；梦见亡人一般至少 7 张、自己的亲人最好 21 张及以上；确定不了数量就坚持给自己的要经者念），再说个人具体张数以师父开示或义工面谈为准；只回「因人而异／建议联系义工」就是没有回答。**例外**：佛台请下／结缘／搬迁／换像的张数不给一般说法，一律请义工面谈。');
  } else if (turn.level === 'practising') {
    // 09-16 C2: 「因人而异，请联系义工」 alone is 该答不答. The 《入门手册》 general
    // statement is official and must be written out; it goes BEFORE the
    // 默认不给遍数 line, which the model otherwise reads as covering 张数 too.
    parts.push('【小房子张数】访客问「要念多少张」时，**必须先写出《心灵法门入门手册》的一般说法**：一般初学者先念 4-10 张小房子给自己的要经者；打胎、流产、宫外孕每个胎儿至少 7 张、最好 21 张以上；梦见亡人一般至少 7 张、自己的亲人最好 21 张及以上；确定不了数量就坚持给自己的要经者念。写完这一句，再说「个人具体张数以师父开示或义工面谈为准」。只回「因人而异／建议联系义工」而不给这句一般说法，就是没有回答。');
    parts.push('【本轮判级：practising】访客已在念功课。遍数默认不主动给（张数的一般说法不在此列，见上）；访客问「念几遍／几张」或话题本身是教义数字（小房子规格、礼佛特殊日子、369）时才给，给就按功课卡／《佛学问答》161。');
  }
  if (turn.needsCare) {
