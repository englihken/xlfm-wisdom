...
// `label` is the zh fallback (used when no translator is supplied, e.g. legacy/
// zh-only callers); `key` routes the chip through i18n (inbox.status.*) when a t()
// is passed. zh stays byte-identical to the previous labels.
export const STATUS_META: Record<ThreadStatus, { label: string; key: string; chip: string }> = {
  // chip class names map to the app's pill utilities / inline tokens (globals.css).
  new: { label: '未处理', key: 'inbox.status.new', chip: 'bg-[#EAF1FA] text-[#2E5A8A] border border-[#CBDDF0]' }, // sky
  in_progress: { label: '处理中', key: 'inbox.status.in_progress', chip: 'bg-[#E7F0E0] text-[#3F6B2E] border border-[#CFE3C0]' }, // jade
  replied: { label: '已回复', key: 'inbox.status.replied', chip: 'pill-muted' },
  archived: { label: '已归档', key: 'inbox.status.archived', chip: 'pill-muted' },
};

export const STATUS_KEYS: ThreadStatus[] = ['new', 'in_progress', 'replied', 'archived'];
// Pass a translator (client useT / server getServerT) to localize; omit for the zh fallback.
export const statusLabel = (s: string, t?: TFunc): string => {
  const meta = STATUS_META[s as ThreadStatus];
  if (!meta) return s;
  return t ? t(meta.key) : meta.label;
};
...
export const KIND_KEY: Record<ThreadKind, string> = {
  form: 'inbox.kind.form',
  internal: 'inbox.kind.internal',
};
export const kindLabel = (k: string, t?: TFunc): string => {
  const key = KIND_KEY[k as ThreadKind];
  return t && key ? t(key) : (KIND_LABEL[k as ThreadKind] ?? k);
};
